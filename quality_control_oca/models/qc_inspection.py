...
class QcInspection(models.Model):
    ...
    def _unlink_except_autogenerated_and_non_draft(self):
        inspections_data = self.read(["auto_generated", "state"])

        if (
            not self.env.is_superuser()
            and any(data["auto_generated"] for data in inspections_data)
        ):
            raise exceptions.UserError(
                self.env._("You cannot remove an auto-generated inspection.")
            )

        if any(data["state"] != "draft" for data in inspections_data):
            raise exceptions.UserError(
                self.env._(
