...
class QcInspection(models.Model):
    ...
    def _unlink_except_autogenerated_and_non_draft(self):
        can_remove_autogenerated = self.env.is_superuser() or self.user_has_groups(
            "quality_control_oca.group_quality_control_manager"
        )

        if not can_remove_autogenerated and self.filtered("auto_generated"):
            raise exceptions.UserError(
                self.env._("You cannot remove an auto-generated inspection.")
            )

        if self.filtered(lambda inspection: inspection.state != "draft"):
            raise exceptions.UserError(
                self.env._(
